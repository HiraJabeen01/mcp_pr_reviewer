...
class ConnectionManager:
    ...
    async def connect_to_server(self, server_key: str):
        ...
        if config["type"] == "stdio":
            ...
            if HAS_MCP_V2:
                import httpx2
                http_client = httpx2.AsyncClient(headers=config.get("headers"))
                await self.exit_stack.enter_async_context(http_client)
                context = streamable_http_client(url=config["url"], http_client=http_client)
                print(f"Connecting to SSE server '{server_key}' with streamable_http_client...")
                streams = await self.exit_stack.enter_async_context(context)
        ...
        try:
            ...
            print(f"[Tools] Tools available for {server_key}:")
        ...
        try:
            ...
            print(f"[Prompts] Prompts available for {server_key}:")
    ...
    async def _run_session(self, read_stream, write_stream, get_session_id):
        print("[Session] Initializing MCP session...")
